...
En macOS, `make windows-command` imprime el comando completo. En Windows se pega ese comando en PowerShell como Administrador; `build-windows-pilot.ps1` instala mediante `winget` cualquier dependencia ausente (.NET SDK 10, Visual Studio 2022 Build Tools con C++/Spectre, Windows SDK 26100 y WDK 26100), crea o reutiliza el certificado de desarrollo y compila automáticamente el `.env.infra` local dentro del servicio. No se pasa ningún parámetro y el primer arranque registra la llave automáticamente.
...
`build-windows.ps1` recompila el WFP, firma el SYS y el catálogo con el certificado `Development`, valida el catálogo exacto y marca el ZIP como `Test`. No distribuyas ese ZIP. Si el juego o Easy Anti-Cheat no abre, ejecuta `clean-windows-pilot.ps1` desde la raiz del repositorio en PowerShell como Administrador; el script retira el paquete WFP incluso si falla el desinstalador instalado, elimina el certificado de desarrollo, deshabilita Test Mode y solicita el reinicio. Si Secure Boot fue deshabilitado en el BIOS, debe habilitarse manualmente. El flujo `Microsoft` sigue siendo obligatorio para otros usuarios o una release pública.
