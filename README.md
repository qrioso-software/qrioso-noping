...
`make windows-command` imprime exactamente ese mismo comando. Si faltan Git for Windows, .NET SDK 10, Visual Studio 2022 Build Tools con C++/Spectre, Windows SDK 26100 o WDK 26100, el flujo los instala mediante `winget`; después actualiza `main`, reutiliza o crea el certificado local de desarrollo, habilita Test Mode y ejecuta el build completo. La preparación inicial del toolchain nativo puede tardar varios minutos. En el primer arranque, el servicio registra automáticamente la llave compilada y guarda el estado con DPAPI.
