...
`make windows-command` imprime exactamente ese mismo comando. Si Git for Windows no existe, el comando lo instala mediante `winget`; después actualiza `main` y ejecuta el piloto. Si falta .NET SDK 10, `build-windows-pilot.ps1` también lo instala mediante `winget`, reutiliza o crea el certificado local de desarrollo, habilita Test Mode y ejecuta el build completo. En el primer arranque, el servicio registra automáticamente la llave compilada y guarda el estado con DPAPI.
